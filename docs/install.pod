...
Screen 0. This is where I run my web server. I don't run as a daemon so I can see
all the log and error messages output to the screen and I can start/stop the server
as I wish.

Screen 1. I run the Redis server here, again not as a daemon, so I can see all the 
connections and log messages directly to the screen.

Screen 2. Same again, but with beanstalk.

Screen 3. Usually I have a MySQL client into the database. You may prefer to have
a GUI client. This is just my preference.

Screen 4. A vim session into the documentation (you *do* write your documentation first?)

Screen 5. A vim session into the test directory (you *do* do test driven development?)

Screen 6-n. Various vim sessions into source code files etc.

You may prefer to run the server/redis/beanstalk as daemons, but in any case you will
still need a terminal session open to tail the log files, so I don't see the point.
