...
Returns B<status> B<code> B<id> and B<message> (see L<intro.pod>)
...
Note. No message content is required for this call.

=head3 RESPONSE

The server will respond with a B<Server : rooms_status> message




=head2 Server : rooms_status

A B<Server : rooms_status> will be sent after a B<Client : rooms> request.
