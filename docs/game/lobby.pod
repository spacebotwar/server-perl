...
Attempt to log into the server with a B<session> provided earlier, if you don't
have a B<session> then request one with B<Client: get_session>. if you don't make
any server calls within 2 hours a session will time out.
...
The server will respond with a B<Server : login_with_session> message.




=head2 Server : login_with_session

This is a response to B<Client : login_with_session>, 
...
Attempt to log in with a B<username> B<password> combination.
...
A validation code to confirm the users email address. This will generally be provided
as an email to the users registered email address.
...
The server will send a B<Server : login> message with the success or
